...
class VFS(object):
    ...
    def __init__(
        ...
        if realpath:
            ...
            self.histpath = os.path.join(realpath, ".hist")
    ...
    def add(self, src , dst , dst0 )  :
        "a"
        assert src == "/" or not src.endswith("/")
        ...
        if "xdev" in self.flags or "xvol" in self.flags:
            ...
            for le in vfs_ls:
                ...
                if not vn2 or not vn2.get("", uname, True, False):
                    rm1.append(le)
            _ = [vfs_ls.remove(x) for x in rm1]
...
class AuthSrv(object):
    ...
    def _reload(self, verbosity  = 9)  :
        ...
        if enshare:
            ...
            assert db and cur and cur2 and shv
            for row in cur.execute("select * from sh"):
                s_k, s_pw, s_vp, s_pr, s_nf, s_un, s_t0, s_t1 = row

...
class Up2k(object):
    ...
    def register_vpath(
        ...
        if a:
            zs = " ".join(sorted(a))
            zs = zs.replace("90mre.compile(", "90m(")
    ...
    def _build_dir(
        ...
        seen_files = set([x[2] for x in files])
    ...
    def _confirm_chunks(
        ...
        if True:
            ...
            try:
                ...
                return -2, "confirm_chunk, wark(%r)" % (ex,)
            ...
            try:
                ...
                return -2, "confirm_chunk, chash(%s) %r" % (chash, ex)
    ...
    def _handle_rm(
        ...
        if is_dir:
            ...
            g = [(vn, voldir, vpath_dir, adir, [(fn, 0)], [], {})]
        else:
    ...
    def _get_wark(self, cj  )  :
        if len(cj["name"]) > 1024 or len(cj["hash"]) > 512 * 1024:

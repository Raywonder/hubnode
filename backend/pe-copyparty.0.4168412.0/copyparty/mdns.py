...
class MDNS(MCast):
    ...
    def run(self)  :
        ...
        zf = time.time() + 2
        self.probing = zf
        self.unsolicited = [zf, zf + 1, zf + 3, zf + 7]
    ...
    def eat(self, buf , addr  , sck )  :
        ...
        srv  = self.srv[sck] if v6 else self.map_client(cip)
        if not srv:
            return
        ...
        deadline = now + (0.5 if p.header.tc else 0.02)
    ...
    def process(self)  :
        ...
        for srv, deadline in list(self.defend.items()):
            ...
            if self._tx(srv, srv.bp_ip, 0.02):
                self.defend.pop(srv)
